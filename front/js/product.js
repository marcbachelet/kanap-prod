  ...
  .then((data) => {
    ...
    addCart.addEventListener("click", () => {
      ...
      if (
        getBasket == null &&
        productSelect.color == "" &&
        Number(productSelect.quantity == 0)
      ) {
        alert(
          "Veuillez ajouter un kanap et choisir une couleur pour pouvoir commander"
        );
        getBasket = [];
        document.location;
      } else if (
        ...
        getBasket == null &&
        (productSelect.color == "" || Number(productSelect.quantity == 0))
      ) {
        alert(
          "Veuillez remplir le champ manquant pour pouvoir passer votre commande"
        );
        getBasket = [];
        document.location;
      } else if (
      ...
      if (getBasket == null) {
        ...
        for (let item of getBasket) {
          if (
            item.id == productSelect.id &&
            item.color == productSelect.color
          ) {
            confirmBasket();
            return (
              (item.quantity =
                parseInt(productSelect.quantity) + parseInt(item.quantity)),
              localStorage.setItem("basket", JSON.stringify(getBasket))
            );
          }
        }
